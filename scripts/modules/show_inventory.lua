...
local function open_inventory(source, target)
    ...
    local frame = Gui.add_closable_frame(source, { name = 'inventory_gui', caption = 'Inventory' })
...
local function on_selected_tab_changed(event)
    ...
    data.last_tab = element.tabs[element.selected_tab_index].caption
    ...
    if valid then
        ...
        local panel_type = target_inventories[data.last_tab]()

        redraw_inventory(frame, player, target, data.last_tab, panel_type)
    end
end

local function on_gui_closed(event)
    local element = event.element
    if not element or not element.valid or element.name ~= 'inventory_gui' then
        return
    end
...
local function update_gui(event)
    ...
    for watcher_idx, _ in pairs(watchers) do
        ...
        if success then
            local frame = get_active_frame(watcher)
    ...
    if validate_player(player) then
        ...
        if valid then
            if target_player == opened then
                return player.print('You are already viewing this players inventory.', { r = 1, g = 0.5, b = 0 })
            end
        end
        ...
        if validate_player(target_player) then
            ...
            player.print('Please type a name of a player who is connected.', { r = 1, g = 0.5, b = 0 })
        end
    else
        return
    end
...
function get_active_frame(player)
...
fsrc.add(defines.events.on_player_main_inventory_changed, update_gui)
fsrc.add(defines.events.on_player_gun_inventory_changed, update_gui)
fsrc.add(defines.events.on_player_ammo_inventory_changed, update_gui)
fsrc.add(defines.events.on_player_armor_inventory_changed, update_gui)
fsrc.add(defines.events.on_player_trash_inventory_changed, update_gui)
Gui.on_closed(on_gui_closed)
Gui.on_selected_tab_changed('tabbed_pane', on_selected_tab_changed)
fsrc.add(defines.events.on_pre_player_left_game, on_pre_player_left_game)
